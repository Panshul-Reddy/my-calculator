...
class TestCLIIntegration:
    ...
    def run_cli(self, *args):
        ...
        # Prefer running as a module to avoid filename/case issues
        module_cmd = [sys.executable, "-m", "src.cli"] + list(args)
        try:
            result = subprocess.run(module_cmd, capture_output=True, text=True, cwd=".")
            # if module invocation worked or failed for reasons other than not-found,
            # return the result so tests see the accurate behavior.
        ...
        # Fallback: try explicit script file paths (case-insensitive filesystems)
